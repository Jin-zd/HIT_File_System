...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    int sz;
    /* 填充super信息 */
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &sz);
    super.sz_io = sz;
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &sz);
    super.sz_disk = sz;
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    char buf_512[512];
    ddriver_read(super.driver_fd, buf_512, 512);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry dentry;
    memcpy(&dentry, buf_512, sizeof(struct demo_dentry));
...
static int demo_getattr(const char* path, struct stat *stbuf)
{
    if(strcmp(path, "/") == 0)
        ...
        stbuf->st_mode = DEMO_DEFAULT_FILE | S_IFREG;            // 该文件显示为普通文件
    return 0;
}
